...
const tableSchema = new monggose.Schema({
    number: {
        type: String,
    ...
    rotation : {
        ...
        validate : {
            ...
            message: 'La rotation doit petre entre 0 et 360°'
        }
    },
    dimensions : {
        width : {
    ...
    status : {
        type: String,
        enum : ['free', 'occupied', 'reserved', 'cleaning','out_of_order'],
        default: 'free'
    },
    isActive : {
        type: Boolean,
...
const obstacleSchema = new mongoose.Schema({
    type : {
        ...
        enum: {
            values : ['wall', 'column', 'bar', 'kitchen_door', 'entrance', 'window','stairs','door', 'other'],
            message : 'Typed\'obstacle invalide'
        }
    },
    name : {
        type: String,
...
// Schéma principla pour le plan de salle
...
const floorPlanSchema = new mongoose.Schema({
  ...
  floorPlanSchema.index({restaurantId : 1, restaurantId: 1});

  // Middleware pour checker qu'il n'y a qu'un seul plan par défaut par restaurant
  floorPlanSchema.pre('save', async function(next){
    if(this.isDefault && this.isModified('isDefault')){
        // Retire le statut défaut des autres plans du même restaurant
  ...
  // Méthode pour valider qu'il n'y a pas collision dentre tables
  ...
  // Validation personnalisée pour checker que les numéros de tables sont uniques
